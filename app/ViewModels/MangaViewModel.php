...
{
    public function __construct(public array $popularMangas, public array $lastUpdateMangas, public array $recentlyAdded) {}
    ...
    private function getCoverUrl(array $mangaAttributes): string
    {
        if (isset($mangaAttributes['relationships'])) {
            $coverFileName = collect($mangaAttributes['relationships'])
    ...
    {
        return $mangaAttributes['attributes']['title']['es'] ?? $mangaAttributes['attributes']['title']['en'] ?? array_values($mangaAttributes['attributes']['title'])[0];
    }

    private function getTags(array $mangaAttributes): Collection
    {
        return collect($mangaAttributes['attributes']['tags'])->pluck('attributes.name')->flatten(1);
    }

    private function getDescription(array $mangaAttributes): string
    {
        $descriptions = $mangaAttributes['attributes']['description'] ?? [];
        return $descriptions['es'] ?? $descriptions['en'] ?? array_values($descriptions)[0] ?? 'No existe descripción';
    }
}
